...
截止目前为止最新的 Ren'Py 版本（8.5.2）使用的 Python 语法已经和本游戏的 Ren'Py 有部分区别，使得浮点数的计算出现不一致：
